...
router.get("/:id", (req, res) => {
  db("cohorts")
    ...
    .catch(err => {
      res
        .status(500)
        .json({ message: "Error retrieving the specified cohort data." });
    });
});
...
router.get("/:id/students", (req, res) => {
  db("cohorts")
    ...
    .catch(err => {
      res
        .status(500)
        .json({ message: "Error retrieving the specified cohort's students." });
    });
});
...
router.post("/", (req, res) => {
  if (!req.body.name) {
    res
      .status(400)
      .json({ message: "Please Provide a name for the new cohort." });
  } else {
    db("cohorts")
      .insert(req.body, "id")
      .then(ids => {
  ...
  db("cohorts")
    ...
    .then(updates => {
      if (updates > 0) {
        res
          .status(200)
          .json({ message: `${updates} cohort successfully updated.` });
      } else {
        res
          .status(404)
          .json({ message: "The specified cohort does not exist." });
  ...
  db("cohorts")
    ...
    .then(count => {
      if (count > 0) {
        res
          .status(200)
          .json({ message: `${count} cohort successfully deleted.` });
      } else {
        res
          .status(404)
          .json({ message: "The specified cohort does not exist." });
